...
- `src/completion.js` / `src/hover.js` / `src/definition.js` / `src/references.js` / `src/semanticTokens.js` — the IntelliSense providers built on top of the index and `semanticAnalysis.js`; `semanticTokens.js` colors resolved template usages (`class`) and resolved slot usages (`variable`); `references.js` searches every indexed file's current text (open buffer if open, disk otherwise) for usages of a template name, powering "Find All References" and Ctrl-click-on-declaration.
